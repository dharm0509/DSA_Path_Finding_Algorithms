...
pygame.display.set_caption("Dijkstra PathFinding Algorithm")  # title
...
def dijkstra(draw, grid, start, end):
    ...
    while not open_set.empty():
        ...
        current = open_set.get()[2]         # selecting the shortest path node from the open-set i.e. the visited nodes
        open_set_hash.remove(current)

        if current == end:                         # if reached to the end, construct the path using the came_from list
        ...
        for neighbor in current.neighbors:
            ...
            if temp_g_score < g_score[neighbor]:
                came_from[neighbor] = current     # the neighbor came from current
                g_score[neighbor] = temp_g_score  # g-score to reach from current to neighbor was temp_g_score
